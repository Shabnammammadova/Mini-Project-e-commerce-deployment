...
export const Product = ({product}:{product:IProduct & {category: Category}}) => {
  ...
  <div className="bg-[#F3E6DA] h-[300px] flex items-center justify-center rounded-t-xl relative">
    ...
    <div className="px-4 py-1 flex flex-col items-center gap-2 rounded-b-xl absolute top-10 right-0">
      <Button className="bg-transparent text-black border-transparent hover:bg-green-600">
        <Link href={`/edit?id=${product.id}&name=${product.name}&price=${product.price}&imageUrl=${product.imageUrl}`}>
          <Pencil />
        </Link>
      </Button>
      <Button className="bg-transparent text-black border-transparent hover:bg-red-600" onClick={() => handleDelete(product.id)}>
        <Trash />
      </Button>
    </div>
  </div>
