...
export const Product = ({product}:{product:IProduct & {category: Category}}) => {
  ...
  return (
    ...
    <div>
      <div className="xl:w-[30em]  sm:w-[25em] bg-white shadow-md rounded-xl duration-500">
        ...
        <Link key={product.id} href={`/filter/product/${product.id}`}>
          ...
          <p className="text-lg font-extrabold text-black truncate block capitalize absolute top-0 left-0 mx-2">{product.category.name}</p>
        ...
        <div className="px-4 xl:w-[30em] sm:w-[20em] flex items-center justify-between rounded-b-xl bg-[#F3E6DA]">
